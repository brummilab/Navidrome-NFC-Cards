...
design/gehaeuse.scad (v6).  Nutzt trimesh + manifold3d für die
Boolean-Operationen.
...
# ── Primitive ────────────────────────────────────────────────
def rbox(w, d, h, r=4):
    """Abgerundete Box (z = 0..h), wie OpenSCAD 2D-hull + extrude."""
...
def cube(x, y, z, w, d, h):
    """Achsenparalleler Quader mit Min-Ecke bei (x, y, z)."""
...
def lid():
    ...
    cuts = [
        cube(sx, sy, -0.1, skirt_cav_w, skirt_cav_d, SKIRT_H + 0.1),  # Rock-Hohlraum
        pocket,                                                       # Reader-Mulde
        cube(LID_W / 2 - 8, py - 0.1, SKIRT_H - 4,                    # Kabel
             16, POCKET_WALL + 0.2, 4.2),
        cube(LID_W / 2 - 13, -0.1, LID_H - 3, 26, 11, 3.1),          # Griffmulde
    ]
